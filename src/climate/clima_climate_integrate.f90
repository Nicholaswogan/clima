...
contains
  ...
  module function evolve_dop853(self, filename, tstart, T_start, t_eval, overwrite, err) result(success)
    ...
    ! initialize
    call dop%initialize(fcn=rhs_dop853, n=v%nz, status_ok=status_ok)
    ...
    do ii = 2, size(t_eval_1)
      ...
      call dop%initialize(fcn=rhs_dop853, n=v%nz, hinitial=hcur, status_ok=status_ok)
      dop%c => self
